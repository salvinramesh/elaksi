...
export default function Account() {
  ...
  async function saveAddr() {
    try {
      ...
      // if we set default, server clears others — refresh list to reflect order
      if (saved.isDefault) {
        const list = await api('/api/addresses');
        setAddresses(list);
      }
        ...
        right={
          <div className="flex gap-2">
            {!editing ? (
              ...
              <>
                <button className="btn btn-outline" onClick={() => { setEditing(false); setName(me.name || ''); setPhone(me.phone || ''); }}>
                  Cancel
                </button>
        ...
        {addrEditingId && (
          <div className="mb-4 rounded-xl border p-4 bg-amber-50/40">
            ...
            <div className="mt-3 flex gap-2">
              <button className="btn btn-outline" onClick={cancelAddr}>
                Cancel
              </button>
              <button className="btn btn-primary" onClick={saveAddr}>
                Save Address
              </button>
            </div>
          </div>
            ...
            {addresses.map((a) => (
              <li key={a.id} className="rounded-xl border p-3">
                ...
                <div className="mt-2 flex gap-2">
                  <button className="btn btn-outline" onClick={() => beginEditAddr(a)}>
                    Edit
                  </button>
                  <button className="btn btn-outline" onClick={() => removeAddr(a.id)}>
                    Delete
                  </button>
                </div>
              </li>
